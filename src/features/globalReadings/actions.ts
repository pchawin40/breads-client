...
export const toggleFavorite = ({
  id,
  user_id,
}: FavoriteState): ReadingActionTypes => ({
...
export const removeReadings = ({
  reading_id,
  user_id,
}: RemoveReadingState): ReadingActionTypes => ({
  type: REMOVE_READING,
  payload: {
    reading_id,
    user_id,
...
export const postNewReading =
  (
    url: string,
    tags: string
  ): ThunkAction<
    Promise<AlertActionTypes | void>,
    RootState,
    unknown,
    Action<string>
  > =>
  async (
    ...
    if (validator.isURL(url)) {
      ...
      dispatch(
        addAlert({ message: "Try again with a valid URL", type: "danger" })
      );
    }
  };

export const removeUserReading =
  (
    user_id: any,
    reading_id: any
  ): ThunkAction<
    Promise<PromiseReturnTypes>,
    RootState,
    unknown,
    Action<string>
  > =>
  async (dispatch: AppDispatch): Promise<PromiseReturnTypes> => {
...
export const markFavorite =
  (
    ...
    Action<string>
  > =>
  async (dispatch: AppDispatch, getState: any): Promise<PromiseReturnTypes> => {
    let { currentUser } = getState();
...
export const unfavorite =
  (
    ...
    Action<string>
  > =>
  (dispatch: AppDispatch, getState: any): Promise<PromiseReturnTypes> => {
    let { currentUser } = getState();
    const user_id = currentUser.user.id;
...
export const fetchReadingsIfNeeded =
  (
    list: string,
    id: any
